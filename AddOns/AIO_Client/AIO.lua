...
if AIO_SERVER then
    ...
    function AIO_HANDLERS.Init(player, version, N, addons, cached)
        if(AIO_VERSION ~= version) then
            ...
            print("У вас AIO версии "..AIO_VERSION..", а на сервере "..(version or "нет")..". Нужны одинаковые версии")
            return
        end
        ...
        AIO_INITED = true
        print("AIO версии "..AIO_VERSION.." загружен. Список команд: /aio help")
    end
    ...
    function AIO_HANDLERS.ForceReload(player)
        ...
        print("AIO: принудительная перезагрузка интерфейса")
...
if AIO_MAIN_LUA_STATE then
    ...
    if AIO_SERVER then
        local function OnCommand(event, player, msg)
            ...
            pprint(player, "Неизвестная команда .aio "..tostring(msg))
            cmds.help(player)
            return false
        end
        ...
        function SlashCmdList.AIO(msg)
            ...
            print("Неизвестная команда /aio "..tostring(msg))
            cmds.help()
        end
    end
    ...
    helps.help = "показать этот список"
    function cmds.help(player)
        pprint(player, "Доступные команды:")
        for k,v in pairs(cmds) do
            pprint(player, (AIO_SERVER and '.' or '/').."aio "..k.." - "..(helps[k] or "нет описания"))
        end
    end
    if not AIO_SERVER then
        helps.reset = "сбросить локальный кэш AIO: очищает загруженные аддоны и их переменные, затем перезагружает интерфейс"
        function cmds.reset()
            AIO_RESET()
            ReloadUI()
        end
    end
    helps.trace = "включить/выключить использование debug.traceback или _ERRORMESSAGE"
    function cmds.trace(player)
        AIO_ENABLE_TRACEBACK = not AIO_ENABLE_TRACEBACK
        pprint(player, "трассировка теперь", AIO_ENABLE_TRACEBACK and "включена" or "выключена")
    end
    helps.debug = "включить/выключить показ отладочных сообщений"
    function cmds.debug(player)
        AIO_ENABLE_DEBUG_MSGS = not AIO_ENABLE_DEBUG_MSGS
        pprint(player, "показ отладочных сообщений теперь", AIO_ENABLE_DEBUG_MSGS and "включён" or "выключен")
    end
    helps.pcall = "включить/выключить использование pcall"
    function cmds.pcall(player)
        AIO_ENABLE_PCALL = not AIO_ENABLE_PCALL
        pprint(player, "использование pcall теперь", AIO_ENABLE_PCALL and "включено" or "выключено")
    end
    helps.printio = "включить/выключить вывод всех отправленных и принятых сообщений"
    function cmds.printio(player)
        AIO_ENABLE_MSGPRINT = not AIO_ENABLE_MSGPRINT
        pprint(player, "вывод ввода-вывода теперь", AIO_ENABLE_MSGPRINT and "включён" or "выключен")
    end
end
