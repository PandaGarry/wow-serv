...
-- Локализация: строки берутся из locale\ruRU.lua. Чего в словаре нет —
-- возвращается как есть (английский оригинал), поэтому аддон работает
-- на любом клиенте.
local L = setmetatable({}, { __index = function(_, key)
    local value = BattlePassLocale and BattlePassLocale[key]
    return value or key
end })

-- L() со подстановкой: F("Level %d", 12)
...
local function GetLevelStatusText(level)
    ...
    if status == 0 then
        return "|cff888888" .. L["Locked"] .. "|r"
    elseif status == 1 then
        return "|cff00ff00" .. L["Claim!"] .. "|r"
    elseif status == 2 then
        return "|cff4488ff" .. L["Claimed"] .. "|r"
    elseif status == 3 then
        return "|cffffaa00" .. L["Owned"] .. "|r"
    end
...
function BattlePass_OnClaimResult(sender, args)
    ...
    if success then
        ...
        print("|cff00ff00" .. L["[Battle Pass]"] .. "|r " .. (message or L["Reward claimed!"]))
    else
        print("|cffff0000" .. L["[Battle Pass]"] .. "|r " .. (message or L["Failed to claim reward."]))
    end
...
function BattlePass_OnError(sender, args)
    ...
    local message = args[2] or L["Unknown error"]

    print("|cffff0000" .. L["[Battle Pass Error]"] .. "|r " .. message)
end
...
-- Статические подписи из BattlePass.xml: подставляем перевод один раз
-- при загрузке (XML успевает создаться раньше этого файла).
local function LocalizeStaticTexts()
    local texts = {
        BattlePassFrameHeaderTitle    = "Battle Pass",
        BattlePassProgressLabel       = "Progression",
        BattlePassSyncButtonLabel     = "Sync",
        BattlePassClaimAllButtonLabel = "Claim All",
        BattlePassInfoButtonLabel     = "Info",
    }

    for name, key in pairs(texts) do
...
local function InitializeAddon()
    ...
    if not BattlePassScrollContent then
        print("|cffff0000" .. L["[Battle Pass]"] .. "|r " .. L["ERROR: BattlePassScrollContent not found"])
        return
    end
    ...
    for i = 1, VISIBLE_ITEMS do
        ...
        if item then
            ...
            print("|cffff0000" .. L["[Battle Pass]"] .. "|r " .. F("ERROR: Failed to create item %d", i))
        end
    end
...
function BattlePass_UpdateUI()
    ...
    if BattlePassProgressBar then
        ...
        if progressText then
            if playerData.level >= playerData.maxLevel then
                progressText:SetText(L["MAX LEVEL"])
            else
...
function BattlePass_OnLevelItemClick(self)
    ...
    elseif status == 2 then
        -- Already claimed
        print("|cff888888" .. L["[Battle Pass]"] .. "|r " .. F("Level %d already claimed.", level))
    elseif status == 3 then
        -- Already owns the reward
        print("|cffffaa00" .. L["[Battle Pass]"] .. "|r " .. L["You already own this reward!"])
    elseif status == 0 then
        -- Locked
        print("|cff888888" .. L["[Battle Pass]"] .. "|r " .. F("Reach level %d to claim this reward.", level))
    end
end
...
function BattlePass_OnLevelItemEnter(self)
    ...
    GameTooltip:AddLine(F("Level %d", level), 1, 0.82, 0)
    ...
    local typeName = typeNames[levelData.rewardType] or L["Reward"]
    GameTooltip:AddLine(typeName .. " x" .. levelData.count, 0.5, 0.5, 0.5)

    -- Status (using server status if available)
    local status = GetLevelStatus(level)
    if status == 0 then
        GameTooltip:AddLine(F("Locked - Reach level %d", level), 1, 0, 0)
    elseif status == 1 then
        GameTooltip:AddLine(L["Click to claim!"], 0, 1, 0)
    elseif status == 2 then
        GameTooltip:AddLine(L["Already claimed"], 0.5, 0.5, 1)
    elseif status == 3 then
        GameTooltip:AddLine(L["You already own this reward"], 1, 0.7, 0)
    end
...
function BattlePass_ShowXPGain(amount)
    if BattlePassXPGainText then
        BattlePassXPGainText:SetText("|cff00ff00+" .. amount .. " " .. L["XP"] .. "|r")
    end
...
function BattlePass_ShowLevelUp(newLevel)
    ...
    -- Print message
    print("|cffff8000" .. L["[Battle Pass]"] .. "|r " .. F("Level %d reached!", newLevel))
end
...
function BattlePass_ShowInfo()
    print("|cffff8000" .. L["[Battle Pass]"] .. "|r " .. L["Season 1"])
    print("|cff00ff00" .. L["Level:"] .. "|r " .. playerData.level .. " / " .. playerData.maxLevel)
    print("|cff00ff00" .. L["XP:"] .. "|r " .. playerData.currentExp .. " / " .. playerData.expRequired)
    print("|cff00ff00" .. L["Total XP:"] .. "|r " .. playerData.totalExp)
    ...
    print("|cff00ff00" .. L["Claimed Rewards:"] .. "|r " .. claimed .. " / " .. playerData.level)
end
